<template>
  <section>
  <v-img :src="profil.image" gradient="to top, rgba(255, 255, 255), rgba(0, 0, 0, 0.1)" contain>
    <v-layout align-center justify-center row wrap fill-height>
      <v-flex xs12 text-xs-center class="display-2 font-weight-bold">
        <h1 class="display-2 font-weight-bold mb-3 text-xs-center"> {{profil.judul_header}}</h1>
      <h4 class="title align-center text-xs-center">{{profil.slogan_header}}</h4>
      </v-flex>
    </v-layout>
  </v-img>
  </section>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        links: [
          ...
          { title: 'Profil', to: '/profil', icon: 'mdi-tag-plus' },
    ...
    methods: {
      listHeader() {
          db2.collection('header').doc('profil').get().then((doc) => {
            const data = {
              id: doc.id,
